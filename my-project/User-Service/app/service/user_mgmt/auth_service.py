...
from db.models.user import User, UserInRole, Role
from api.schemes.user import LoginRequest, LoginResponse, RefreshTokenRequest, RefreshTokenResponse
from service.user_mgmt.auth import create_access_token, create_refresh_token, verify_token
from service.user_mgmt.password import verify_password
...
class AuthService:
    ...
    def register(self, username: str, email: str, password: str) -> LoginResponse:
        ...
        # 创建新用户
        from service.user_mgmt.password import hash_password
        user = User(
            id=str(uuid.uuid4()),
            name=username,
            email=email,
        ...
        # 返回登录响应
        from api.schemes.user import LoginRequest
